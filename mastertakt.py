...
while (True):
	try:
		...
		# neu GPIO17 als Eingang fuer Start-Taste
			...
			# file-operationen
			# lesen, was webserver geschrieben hat
			
#			outdatafile = open('outfile.txt','r')
#			outdata = outdatafile.readlines()
#			print('outdata read : ',outdata)
			# daten als strings
#			if len(outdata) is not 0:
			
#				out_startdaten = int(outdata[0],10)
#				out_lbdaten = int(outdata[1],16)
#				out_hbdaten = int(outdata[2],16)
#				out_list = outdata[3].split(" ") # list
			
#				out_data = [int(d,16) for d in out_list]
			
			#print('out_data: ',out_data)
			
#			in_startdaten = spi_shift_data()		
			
			#print('shift: in_startdaten: ',in_startdaten, ' in_lbdaten: ',in_lbdaten,' in_hbdaten: ',in_hbdaten)
			
#			print('shift: in_data: ',in_data)
			
#			indatastring = ' '.join([str(d) for d in in_data])
			
#			indatafile = open('infile.txt','w')
			
#			indatafile.write(str(in_startdaten)+'\n' + str(in_lbdaten) +'\n' + str(in_hbdaten)+'\n' + indatastring)
			#indatafile.write(str(in_lbdaten)+'\n')
			#indatafile.write(str(in_hbdaten)+'\n')
			#indatafile.write(indatastring)
...
#			masterinput = spi_shift_data()
		...
		#	indatafile = open('infile.txt','w')
			...
			while (i > 0):
				...
				try:
					...
					r = requests.get('http://0.0.0.0:5000/twi?pw=ideur00&twitakt=1&b0=24&b1=91&b2=24&b3=91')
	
					r.raise_for_status()
					mastercount = mastercount + 1 # anz Aufrufe von master
					#print(type(r))
					if terminalda is not None:
						print('nach request mastercount: ',mastercount)
						print('status: ',r.status_code)
						#print('status: ',requests.exceptions)
		...
		#	indatafile.write(str(mastercount))
			#indatafile.write(str(in_lbdaten)+'\n')
			#indatafile.write(str(in_hbdaten)+'\n')
			#indatafile.write(indatastring)
		...
		#	indatafile.close()
			...
			subprocess.call(["sleep 5", "DISPLAY=:0","xdotool", "key", "ctrl+F5"])
			
			#GPIO.output(27,0)
			#pass
	except KeyboardInterrupt:
			sys.exit()
			print("cleanup in while")
			GPIO.cleanup()
